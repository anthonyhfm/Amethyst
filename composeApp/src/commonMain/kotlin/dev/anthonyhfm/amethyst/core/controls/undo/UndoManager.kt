...
object UndoManager {
        ...
        if (undoStack.isNotEmpty()) {
            ...
            when (action) {
                ...
                is UndoableAction.MultiChainDeviceCreation -> {
                    // Remove all created devices in descending index order to avoid shifting issues
                    action.creations.sortedByDescending { it.creationIndex }.forEach { creation ->
                        val deviceIndex = creation.parent.devices.value.indexOfFirst {
                            it.selectionUUID == creation.device.selectionUUID
                        }
                        if (deviceIndex != -1) {
                            creation.parent.remove(deviceIndex, fromUser = false)
                        }
                    }
                    redoStack.add(action)
                }
                ...
                is UndoableAction.TimelineMultiTrackChange -> {
                    restoreTimelineTracks(action.beforeTracks)
                    SelectionManager.replaceSelections(action.beforeSelections)
                    redoStack.add(action)
                }
    ...
    fun redo() {
        if (redoStack.isNotEmpty()) {
            ...
            when (action) {
                ...
                is UndoableAction.MultiChainDeviceCreation -> {
                    // Re-insert all created devices in ascending creationIndex order
                    action.creations.sortedBy { it.creationIndex }.forEach { creation ->
                        creation.parent.add(creation.device, atIndex = creation.creationIndex, fromUser = false)
                    }
                    undoStack.add(action)
                }
                ...
                is UndoableAction.TimelineMultiTrackChange -> {
                    restoreTimelineTracks(action.afterTracks)
                    SelectionManager.replaceSelections(action.afterSelections)
                    undoStack.add(action)
                }
    ...
    private fun restoreTimelineTracks(snapshots: Map<Int, dev.anthonyhfm.amethyst.timeline.data.TimelineTrack<*>>) {
        val restored = TimelineRepository.tracks.value.toMutableList()
        snapshots.forEach { (trackIndex, track) ->
            if (trackIndex in restored.indices) restored[trackIndex] = track
        }
        TimelineRepository.updateTracksSnapshot(restored)
    }
