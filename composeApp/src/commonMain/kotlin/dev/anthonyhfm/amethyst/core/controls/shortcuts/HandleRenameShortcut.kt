...
fun handleRenameShortcut(@Suppress("UNUSED_PARAMETER") keyEvent: KeyEvent? = null): Boolean {
    ...
    // GroupChainItem rename
    val groupItem = selections.filterIsInstance<Selectable.GroupChainItem>().firstOrNull()
    if (groupItem != null) {
        SelectionManager.renameRequest.tryEmit(
            SelectionManager.RenameTarget.GroupItem(
    ...
    if (track != null) {
        SelectionManager.renameRequest.tryEmit(SelectionManager.RenameTarget.Track(
