...
object CompositionAutomationParameters {
    fun forNode(node: CompositionNode): List<CompositionAutomationParameter> = when (node.state) {
        ...
        is CutoutNodeState -> listOf(float("radius","Radius",0f,1f,{(it.state as CutoutNodeState).radius}){n,v->n.copy(state=(n.state as CutoutNodeState).copy(radius=v))},float("feather","Feather",0f,1f,{(it.state as CutoutNodeState).feather}){n,v->n.copy(state=(n.state as CutoutNodeState).copy(feather=v))})
