...
/**
 * A timeline track that contains MIDI entries for lights sequencing.
 * This track type is specifically for lights/launchpad control using MIDI-style notation.
 */
class LightsTimelineTrack : TimelineTrack<MidiEntry>() {
    ...
    fun addEntry(entry: MidiEntry) {
        ...
        println("LightsTimelineTrack: Added MIDI entry '${entry.name}' at ${entry.startTimeMs}ms with ${entry.notes.size} notes")
    }

    /**
     * Create a new MIDI entry with notes at a specific position
     */
    fun createEntry(startTimeMs: Long, durationMs: Long, notes: List<MidiNote>, name: String = "Lights Clip") {
        val entry = MidiEntry(
